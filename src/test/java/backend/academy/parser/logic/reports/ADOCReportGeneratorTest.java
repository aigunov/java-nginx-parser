...
public class ADOCReportGeneratorTest {
    ...
    public void testGenerateReport() {
        ...
        assertTrue(reportContent.contains("== Запрашиваемые ресурсы"), "Отчет должен содержать раздел 'Запрашиваемые ресурсы'");
        assertTrue(reportContent.contains("| `/index.html`       | 5 000"), "Отчет должен содержать количество запросов для /index.html");
        assertTrue(reportContent.contains("| `/about.html`       | 2 000"), "Отчет должен содержать количество запросов для /about.html");

        assertTrue(reportContent.contains("== Статистика по кодам ответа"), "Отчет должен содержать раздел 'Статистика по кодам ответа'");
        assertTrue(reportContent.contains("| 200 | OK                     | 8 000"), "Отчет должен содержать статус 200 с количеством 8000");
        assertTrue(reportContent.contains("| 404 | NOT_FOUND              | 1 000"), "Отчет должен содержать статус 404 с количеством 1000");
        assertTrue(reportContent.contains("| 500 | INTERNAL_SERVER_ERROR  | 500"), "Отчет должен содержать статус 500 с количеством 500");

        assertTrue(reportContent.contains("== Дополнительная информация"), "Отчет должен содержать раздел 'Дополнительная информация'");
