...
public class MDReportGeneratorTest {
    ...
    public void testGenerateReport() {
        ...
        assertTrue(reportContent.contains("#### Общая информация"),
            "Отчет должен содержать заголовок 'Общая информация'");
        assertTrue(reportContent.contains("#### Запрашиваемые ресурсы"),
            "Отчет должен содержать заголовок 'Запрашиваемые ресурсы'");
        ...
        assertTrue(reportContent.contains("|  `/index.html`  | 5 000 |"),
            "Отчет должен содержать данные для ресурса /index.html");
        assertTrue(reportContent.contains("|  `/about.html`  | 2 000 |"),
            "Отчет должен содержать данные для ресурса /about.html");
        assertTrue(reportContent.contains("|  `/contact.html`  | 1 000 |"),
            "Отчет должен содержать данные для ресурса /contact.html");

        assertTrue(reportContent.contains("| 200 | OK                     | 8 000 |"),
            "Отчет должен содержать код ответа 200");
        assertTrue(reportContent.contains("| 404 | NOT_FOUND              | 1 000 |"),
            "Отчет должен содержать код ответа 404");
        assertTrue(reportContent.contains("| 500 | INTERNAL_SERVER_ERROR  | 500 |"),
            "Отчет должен содержать код ответа 500");

        assertTrue(reportContent.contains("| Количество уникальных ресурсов | 3 |"),
            "Отчет должен содержать количество уникальных ресурсов");
        assertTrue(reportContent.contains("__*Размеры ответов представлены в байтах*__"),
            "Отчет должен указывать, что размеры ответов представлены в байтах");
    }
}
